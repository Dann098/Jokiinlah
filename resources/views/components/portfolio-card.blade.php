@props(['portfolio'])
@php($thumbnailUrl = $portfolio->resolvedThumbnailUrl())
<article class='surface-card hover-lift overflow-hidden' data-reveal>
    <a href='{{ route('portfolios.show', $portfolio) }}' class='block'>
        @if($thumbnailUrl)
            <img src='{{ $thumbnailUrl }}' alt='Pratinjau {{ $portfolio->title }}' width='720' height='440' loading='lazy' class='aspect-[16/10] w-full object-cover'>
        @else
