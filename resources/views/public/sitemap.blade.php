...
@foreach([route('home'), route('services.index'), route('portfolios.index'), route('articles.index'), route('faq.index'), route('contact.index'), route('privacy'), route('terms')] as $url)
    <url><loc>{{ $url }}</loc></url>
@endforeach
